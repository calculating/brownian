...
function spontaneous_generation(event) {
    ...
    for (deus_ex_machina = 0; deus_ex_machina < air; deus_ex_machina++) {
        oxygen.push([cX + Math.random() * 1, cY + Math.random() * 1, Math.random(), Math.random()]);
    }
    //step()
}
    ...
    for (molecule = 0; molecule < oxygen.length; molecule++) {
        ...
        for (forces = 0; forces < oxygen.length; forces++) {
            if (forces !== molecule) {
                ...
                bump[0] += ratioX * (1 / Math.pow(distance, 2)) * 10
                bump[1] += ratioY * (1 / Math.pow(distance, 2)) * 10
            }
        }
        ...
        // border tesseract
        if (oxygen[molecule][0] > c.width || oxygen[molecule][0] < 0 || oxygen[molecule][1] > c.height || oxygen[molecule][1] < 0) {
